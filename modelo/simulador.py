...
"""
Simulador de Atratividade e Canibalização para Eletropostos.
Gera a tabela de retornos R_c(S) e o mapeamento de cobertura de POIs.

O efeito da rede já existente está no potencial de instalação (Etapa 1, x4).
Por isso a atratividade parte do potencial (score_base) e desconta apenas a
canibalização entre os candidatos vizinhos que também são instalados:
    R_c(S) = PI_c − Σ perda(vizinhos candidatos em S)
"""
...
def _distancia_rua_aproximada(lat1: float, lon1: float, lat2: float, lon2: float) -> float:
    """Haversine com fator de tortuosidade (1.3) para aproximar distância por ruas."""
    R = 6371000
    phi1, phi2 = math.radians(lat1), math.radians(lat2)
    delta_phi = math.radians(lat2 - lat1)
    delta_lambda = math.radians(lon2 - lon1)
...
def pré_computar_cenarios(df_cand: pd.DataFrame, dados_evs: List[Dict], df_pois: pd.DataFrame,
                          raio_influencia_m: int = 1500, max_vizinhos: int = 4,
                          raio_cobertura_m: int = 800) -> Tuple[List[Dict], Dict]:
    """
    Retorna:
      1. cenarios_cplex: tabela de retornos R_c(S)
      2. dados_cobertura: mapeamento de POIs aos candidatos que os cobrem
    """
    if df_cand.empty:
        return [], {}

    # Fração máxima do potencial que um vizinho candidato muito próximo "rouba".
    FATOR_CANIBALIZACAO_CAND = 0.40
    # Impacto total até 300 m; decai linearmente até zero em raio_influencia_m.
    ZONA_IMPACTO_TOTAL_M = 300.0
    ...
    for idx, row in df_cand.iterrows():
        candidatos.append({
            ...
            'lat': row['Lat_Centroide'],
            'lng': row['Lng_Centroide'],
            'score_base': row['Score_Estimado'],   # potencial de instalação (PI)
        })

    def forca(dist: float, raio_max: float, zona: float) -> float:
        if dist <= zona:
            return 1.0
        if dist >= raio_max:
            return 0.0
        return 1.0 - ((dist - zona) / (raio_max - zona))

    # --- PARTE 1: RETORNOS R_c(S) ---
    cenarios_cplex = []
    for c in candidatos:
        # A rede existente já está no PI: o retorno próprio parte do potencial.
        R_c_proprio = c['score_base']

        vizinhos = []
        for outro in candidatos:
            if c['id'] == outro['id']:
                continue
            d = _distancia_rua_aproximada(c['lat'], c['lng'], outro['lat'], outro['lng'])
            if d <= raio_influencia_m:
                impacto = c['score_base'] * FATOR_CANIBALIZACAO_CAND * forca(d, raio_influencia_m, ZONA_IMPACTO_TOTAL_M)
                if impacto > 0:
                    vizinhos.append({'id': outro['id'], 'impacto': impacto})
        vizinhos = sorted(vizinhos, key=lambda x: x['impacto'], reverse=True)[:max_vizinhos]

        combinacoes = []
        for tamanho in range(len(vizinhos) + 1):
            for subset in itertools.combinations(vizinhos, tamanho):
                combinacoes.append(list(subset))

        for subset in combinacoes:
            perda = sum(v['impacto'] for v in subset)
            subset_ids = tuple(sorted(v['id'] for v in subset))
            cenarios_cplex.append({
                'candidato_id': c['id'],
                'configuracao_ativa': subset_ids,
                'retorno_liquido': R_c_proprio - perda,
            })

    # --- PARTE 2: MAPEAMENTO DE COBERTURA DE POIs (R3 e R4) ---
    total_pois = len(df_pois)
    cobertos_existentes = 0
    mapeamento_cobertura = {}
    ...
    for idx_p, row_p in df_pois.iterrows():
        ...
        for ev in dados_evs:
            if 'location' in ev:
                d = _distancia_rua_aproximada(lat_p, lng_p, ev['location']['latitude'], ev['location']['longitude'])
                if d <= raio_cobertura_m:
                    coberto_por_existente = True
                    break

        if coberto_por_existente:
            cobertos_existentes += 1
        else:
            cobridores = [c['id'] for c in candidatos
                          if _distancia_rua_aproximada(lat_p, lng_p, c['lat'], c['lng']) <= raio_cobertura_m]
            if cobridores:
                mapeamento_cobertura[poi_id] = cobridores

    dados_cobertura = {
        'total_pois': total_pois,
        'cobertos_existentes': cobertos_existentes,
        'mapeamento_candidatos': mapeamento_cobertura,
    }
    return cenarios_cplex, dados_cobertura
...
def mock_resolver_cplex(cenarios_cplex: List[Dict], dados_cobertura: Dict):
    pass  # mantido apenas para compatibilidade de imports