...
#     # ==========================================
#     # 1. CRIAÇÃO DOS GRUPOS DE CAMADAS (Com HTML na Legenda)
#     # ==========================================
...
#     icone_ev_html = "<div style='display:inline-flex; align-items:center; justify-content:center; background-color:#2e7d32; border:1px solid cyan; border-radius:50%; width:14px; height:14px; margin-right:4px; box-shadow: 0 0 2px rgba(0,0,0,0.5);'><i class='fa fa-bolt' style='color:cyan; font-size:8px;'></i></div> Eletroposto (concorrência)"
...
#     fg_heatmap = folium.FeatureGroup(name="<span style='background: linear-gradient(to right, blue, lime, red); width: 14px; height: 14px; display: inline-block; margin-right: 4px; border-radius: 2px;'></span> Densidade de demanda", show=False)
...
#     # ==========================================
#     # 2. ADICIONAR ELEMENTOS AOS SEUS RESPECTIVOS GRUPOS
#     # ==========================================
...
#             pane='pois_top_pane',
#             tooltip=f"{row['Nome']} ({row['Tipo']}) - Peso: {row['Peso']:.1f}"
#         ).add_to(fg_pois_dict[cat]) # Adiciona à subcamada correta
...
#             # Distribuição nas camadas corretas
#             if otimizacao_rodou:
#                 if foi_selecionado:
...
#     # ==========================================
#     # 3. ANEXAR GRUPOS AO MAPA NA ORDEM DA LEGENDA
#     # ==========================================
#     # A ordem em que adicionamos ao mapa dita a ordem no menu suspensório!
...
#     # ==========================================
#     # 4. ATIVAR CONTROLE INTERATIVO (Substitui a legenda antiga)
#     # ==========================================
#     # position='bottomright' move o menu exatamente para onde ficava a legenda antiga
#     # collapsed=False faz com que ele inicie aberto, funcionando exatamente como uma legenda visível.
...
#     # 1. Instância base do mapa (CRIAMOS SEM FUNDO INICIALMENTE)
#     mapa = folium.Map(location=[lat, lng], zoom_start=zoom_start, tiles=None)
    
#     # 2. ADICIONAMOS O FUNDO EXPLICITAMENTE COM O NOME DESEJADO
#     folium.TileLayer(
...
#     # ==========================================
#     # 6. ATIVAR CONTROLE INTERATIVO (Substitui a legenda antiga)
#     # ==========================================
#     folium.LayerControl(position='bottomright', collapsed=False).add_to(mapa)
...
#         tiles='CartoDB positron',
#         name='Mapa Base',
#         control=False  # O segredo para ocultar do menu está aqui!
#     ).add_to(mapa)
...
#                 # --- RECUPERANDO O HTML COMPLETO DO POPUP ---
#                 popup_html = f"""
#                 <div style="font-family: Arial, sans-serif; width: 280px;">
#                     <h4 style="margin: 0 0 8px 0; color: #2e7d32; border-bottom: 1px solid #ccc; padding-bottom: 5px;">{nome}</h4>
#                     <p style="margin: 0 0 10px 0; font-size: 13px; font-weight: bold; color: #e65100;">{estrelas}</p>
...
#                     [p_lat, p_lng],
#                     icon=DivIcon(html=html_eletroposto, icon_size=(18, 18), icon_anchor=(10, 10)),
#                     popup=folium.Popup(popup_html, max_width=400), # <- APLICANDO O HTML RESTAURADO
#                     tooltip=f"{nome}"
#                 ).add_to(fg_existentes)
