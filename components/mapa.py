...
#     # --- CAMADA 2: GRADE (Retângulos no chão) ---
#     for _, row in df_cand.iterrows():
#         i, j = row['cell_i'], row['cell_j']
#         c_lat_min = grid['lat_min'] + (i * grid['lat_step'])
#         c_lat_max = c_lat_min + grid['lat_step']
...
#     # --- CAMADA 3: NODOS CANDIDATOS (Marcadores base) ---
#     for _, row in df_cand.iterrows():
#         folium.Marker(
#             [row['Lat_Centroide'], row['Lng_Centroide']],
#             popup=f"<b>CANDIDATO EVCS</b><br>POIs na área: {row['Qtd_POIs']}<br>Score base: {row['Score_Estimado']:.1f}",
...
#     # --- LEGENDA DINÂMICA ---
#     # O ícone do Eletroposto na legenda também foi atualizado para HTML
...
#             # Ícone do Nodo Candidato (Alterado para cinza para indicar "opção latente")
#             folium.Marker(
#                 [row['Lat_Centroide'], row['Lng_Centroide']],
#                 popup=f"<b>CANDIDATO EVCS</b><br>POIs na área: {row['Qtd_POIs']}<br>Score base: {row['Score_Estimado']:.1f}",
#                 tooltip="Nodo candidato otimizado",
#                 icon=folium.Icon(color='lightgray', icon_color='black', icon='wrench', prefix='fa')
#             ).add_to(mapa)
